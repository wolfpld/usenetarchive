/*  $Id: date.c 9082 2010-07-11 09:27:43Z iulius $
**
              ...
              int *values)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        switch (rule->type) {
        case rule::TYPE_DELIM:
            if (*p != rule->delimiter)
                return NULL;
            p++;
            break;
        case rule::TYPE_LOOKUP:
            p = parse_lookup(p, rule, &values[i]);
            break;
        case rule::TYPE_OBS_MONTH:
            p = parse_legacy_month(p, rule, &values[i]);
            break;
        case rule::TYPE_NUMBER:
...
parsedate_rfc5322(const char *date)
{
    ...
    static const struct rule base_rule[] = {
        { rule::TYPE_LOOKUP, 0,   WEEKDAY,  7, 3, 3 },
        { rule::TYPE_DELIM,  ',', NULL,     0, 1, 1 },
        { rule::TYPE_NUMBER, 0,   NULL,     0, 1, 2 },
        { rule::TYPE_LOOKUP, 0,   MONTH,   12, 3, 3 },
        { rule::TYPE_NUMBER, 0,   NULL,     0, 2, 4 },
        { rule::TYPE_NUMBER, 0,   NULL,     0, 2, 2 },
        { rule::TYPE_DELIM,  ':', NULL,     0, 1, 1 },
        { rule::TYPE_NUMBER, 0,   NULL,     0, 2, 2 }
    };
...
parsedate_rfc5322_lax(const char *date)
{
    ...
    static const struct rule base_rule[] = {
        { rule::TYPE_NUMBER,    0,   NULL,  0, 1, 2 },
        { rule::TYPE_OBS_MONTH, 0,   NULL, 12, 3, 3 },
        { rule::TYPE_NUMBER,    0,   NULL,  0, 2, 4 },
        { rule::TYPE_NUMBER,    0,   NULL,  0, 1, 2 },
        { rule::TYPE_DELIM,     ':', NULL,  0, 1, 1 },
        { rule::TYPE_NUMBER,    0,   NULL,  0, 1, 2 }
