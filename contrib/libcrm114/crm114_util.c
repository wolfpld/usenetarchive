			       ...
			       const CRM114_CONTROLBLOCK *cb)
{
  ...
  for (c = 0; c < cb->how_many_classes; c++)
    {
      crm114__strn1cpy(r->cls[c].name, cb->cls[c].name, sizeof(r->cls[c].name));
      r->cls[c].success   = cb->cls[c].success;
      r->cls[c].documents = cb->cls[c].documents;
      r->cls[c].features  = cb->cls[c].features;
    }
}
...
void crm114__result_pR_outcome(CRM114_MATCHRESULT *r)
{
  ...
  for (i = 0; i < ncls; i++)
    {
      ...
      for (j = 0; j < ncls; j++)
	if (j != i)
	  remainder += r->cls[j].prob;
      r->cls[i].pR = crm114__pR(r->cls[i].prob, remainder);
    }
  ...
  for (i = 0; i < ncls; i++)
    if ( !r->cls[i].success)
      remainder += r->cls[i].prob;
  r->overall_pR = crm114__pR(tsprob, remainder);

  // find best match
  bestmatch = 0;
  for (i = 1; i < ncls; i++)
    if (r->cls[i].prob > r->cls[bestmatch].prob)
      bestmatch = i;
...
void crm114_show_result_class(const CRM114_MATCHRESULT *r, int icls)
{
  printf(" %3d %c (%-8s): documents: %d  features: %d  hits: %5d  prob: %.3f  pR: % .3f",
	 icls,
	 (r->cls[icls].success) ? 'S' : 'F',
	 r->cls[icls].name,
	 r->cls[icls].documents,
	 r->cls[icls].features,
	 r->cls[icls].hits,
	 r->cls[icls].prob,
	 r->cls[icls].pR);
  ...
  switch (r->classifier_flags & CRM114_FLAGS_CLASSIFIERS_MASK)
	...
	printf(" chi2: %8.3f", r->cls[icls].u.markov.chi2);
      break;
    case CRM114_HYPERSPACE:
      printf(" radiance: %8.3e", r->cls[icls].u.hyperspace.radiance);
      break;
    case CRM114_ENTROPY:
      printf(" jumps: %3d entropy: %8.3f",
	     r->cls[icls].u.bit_entropy.jumps,
	     r->cls[icls].u.bit_entropy.entropy);
      break;
    case CRM114_FSCM:
      printf(" compression: %8.3f", r->cls[icls].u.fscm.compression);
      break;
    case CRM114_NEURAL_NET:
      printf(" in_class: %8.3f", r->cls[icls].u.neural.in_class);
      break;
    case CRM114_CORRELATE:
      printf(" L1: %d L2: %d L3: %d L4: %d",
	     r->cls[icls].u.bytewise.L1,
	     r->cls[icls].u.bytewise.L2,
	     r->cls[icls].u.bytewise.L3,
	     r->cls[icls].u.bytewise.L4);
      break;
    }
